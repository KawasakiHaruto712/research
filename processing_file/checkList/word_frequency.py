...
tri_gram_path = "../../select_list/checkList/N_gram/trigram.csv"
...
def categorize_com(checklist):
    ...
    name_cate = ["コメント全体", "修正要求", "非修正要求", "修正要求予測", "非修正要求予測"]

    # レビューコメントをカテゴリー毎に分類
    all_com = [row["comment"] for _, row in checklist.iterrows()] # 全てのコメント
    req_com = [row["comment"] for _, row in checklist.iterrows() if row["修正要求"] == 1] # 修正要求と分類したコメント
    not_req_com = [row["comment"] for _, row in checklist.iterrows() if row["修正要求"] != 1] # 非要求と分類したコメント
    req_com_pre = [row["comment"] for _, row in checklist.iterrows() if row["予測"] == 1] # 修正要求と予測したコメント
    not_req_com_pre = [row["comment"] for _, row in checklist.iterrows() if row["予測"] != 1] # 非要求と予測したコメント
    com_cate = [all_com, req_com, not_req_com, req_com_pre, not_req_com_pre]
...
def n_gram_com(format_com):
    ...
    for n_gram_num in range(1, 4):
        ...
        # 頻出したN-gramの結果の内上位10件を抽出
        com_count = Counter(n_gram)
        high_num_n_gram.append(com_count.most_common(10))
