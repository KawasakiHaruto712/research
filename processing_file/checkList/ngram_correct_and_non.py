...
def write_ngram(checklist):
    ...
    correct_type_list = ["correct", "incorrect"]
    ...
    for ngram_i, ngram_type in tqdm(enumerate(ngram_type_list), leave=False, desc="N-gram", total=len(ngram_type_list)):
        ...
        for correct_type in tqdm(correct_type_list, leave=False, desc="correct or incorrect"):
            for request_i, request_type in enumerate(request_type_list):
                column_name = f"{correct_type}_{request_type}"
                
                # コメント毎にN-gramの保存
                comments_ngram, ngram_variations = ngram_division_of_checklist_comments(
                    checklist, (ngram_i+1), correct_type, request_i)
        ...
        ngram_df.to_csv(f"../../select_list/checkList/N_gram/correct_comment/{ngram_type}.csv",
                        index=False,
                        encoding="utf_8_sig")
        
def ngram_division_of_checklist_comments(checklist, n, correct_str, label_int):

    comments_ngram = []
    for _, checklist_row in checklist.iterrows():
        target_row = "n"
        if correct_str == "correct":
            if checklist_row["human"] == label_int and checklist_row["BERT"] == label_int:
                target_row = "y"
        else:
            if checklist_row["human"] == label_int and checklist_row["BERT"] != label_int:
                target_row = "y"
