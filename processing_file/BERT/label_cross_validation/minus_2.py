...
for fold, (train_idx, test_idx) in enumerate(kf.split(df)):
    ...
    # "Code-Review-2"を含むテストデータのみを選択
    test_df = test_df[test_df["text"].str.contains("Code-Review-2|I\swould\prefer\sthat\you\didn't\merge\sthis|Major\ssanity\problems\sfound|Do\snot\ssubmit", regex=True, na=False)]

    # テストデータが空の場合、このfoldをスキップ
    if test_df.empty:
        print(f"Fold {fold}: No test data containing 'Code-Review-2'. Skipping this fold.")
        continue
    ...
    training_args = TrainingArguments(
        output_dir=f'../cross_validation/minus_2/results_fold_{fold}',
        num_train_epochs=3,
    ...
    all_indices.extend(test_df.index.tolist())  # テストデータのインデックスを保存
